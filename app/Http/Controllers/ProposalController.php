...
//                $proposalProduct->discount    = isset($products[$i]['discount']) ? $products[$i]['discount'] : 0;
                ...
                $proposalProduct->hsn_code = $products[$i]['hsn_code'];
            ...
            return redirect()->route('estimate.index', $proposal->id)->with('success', __('Proposal successfully created.'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
...
//                    $proposalProduct->discount    = isset($products[$i]['discount']) ? $products[$i]['discount'] : 0;
    ...
    public function destroy(Proposal $proposal)
    {
        if(\Auth::user()->can('delete proposal'))
        {
            if($proposal->created_by == \Auth::user()->creatorId())
            {
                ...
                return redirect()->route('estimate.index')->with('success', __('Proposal successfully deleted.'));
            }
            else
            {
                return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function customerProposal(Request $request)
    {
        if(\Auth::user()->can('manage customer proposal'))
        {
            ...
            return view('estimate.index', compact('proposals', 'status'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission Denied.'));
    ...
    public function customerProposalShow($ids)
    {
        if(\Auth::user()->can('show proposal'))
        {
            ...
            if($proposal->created_by == \Auth::user()->creatorId())
            {
                ...
                return view('estimate.view', compact('proposal', 'customer', 'iteams'));
            }
            else
            {
                return redirect()->back()->with('error', __('Permission denied.'));
...
//                dd($proposalArr);
                ...
                return redirect()->back()->with('success', __('Estimate successfully sent.') );
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
...
//                dd($proposalArr);
    ...
    public function duplicate($proposal_id)
    {
        if(\Auth::user()->can('duplicate proposal'))
        {
            ...
            return redirect()->back()->with('success', __('Estimate duplicate successfully.'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
        ...
        if(\Auth::user()->can('convert invoice'))
        {
            ...
            if($convertInvoice)
            {
                ...
                foreach($proposalProduct as $product)
                {
                    ...
                    $description= $duplicateProduct->quantity.''.__(' quantity sold in').' ' . \Auth::user()->proposalNumberFormat($proposal->proposal_id).' '.__('Estimate convert to invoice').' '. \Auth::user()->invoiceNumberFormat($convertInvoice->invoice_id);
            ...
            return redirect()->back()->with('success', __('Estimate to invoice convert successfully.'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function statusChange(Request $request, $id)
    {
        ...
        return redirect()->back()->with('success', __('stimate status changed successfully.'));
    }

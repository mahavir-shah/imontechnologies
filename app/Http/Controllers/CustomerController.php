...
class CustomerController extends Controller
{
    ...
    public function create()
    {
        if(\Auth::user()->can('create customer'))
        {
            ...
            $clientType = UserClientDiscount::get()->pluck('client_type', 'discount');
            return view('customer.create', compact('customFields','clientType'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function store(Request $request)
    {
        if(\Auth::user()->can('create customer'))
        {
            ...
            $dealer_type = UserClientDiscount::select('client_type')->where('discount', $request->dealer_category)->first();
            if($total_customer < $plan->max_customers || $plan->max_customers == -1)
            {
                ...
                $customer->gst      =$request->gst_number;
                $customer->pan      =$request->pan_number;
                $customer->website      =$request->website;
                $customer->dealer_category =$dealer_type['client_type'];
                $customer->discount =$request->discount;
                $customer->credit_day      =$request->credit_day;
                $customer->credit_limit      =$request->credit_limit;
                $customer->bank_name      =$request->bank_name;
                $customer->bank_account_name      =$request->account_no;
                $customer->bank_ifsc_code      =$request->ifsc_code;
                ...
                $client->lang = Utility::getValByName('default_language');
                $client->created_by = \Auth::user()->creatorId();
                $client->client_type = intval($request->client_type);
                $client->client_credit_line = $request->credit_limit;
                $client->save();
                //Send Email
    ...
    public function edit($id)
    {
        if(\Auth::user()->can('edit customer'))
        {
            ...
            $customFields = CustomField::where('created_by', '=', \Auth::user()->creatorId())->where('module', '=', 'customer')->get();
            $clientType = UserClientDiscount::get()->pluck('client_type', 'discount');
            $dealer_category = $customer->dealer_category;
            // $catArr = UserClientDiscount::get()->where('client_type',$dealer_category)->pluck('client_type', 'discount');

            return view('customer.edit', compact('customer', 'customFields','clientType','dealer_category'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function update(Request $request, Customer $customer)
    {
        ...
        if(\Auth::user()->can('edit customer'))
        {
            ...
            $customer->bank_ifsc_code    =$request->bank_ifsc_code;
            $customer->created_by       = \Auth::user()->creatorId();
            $customer->billing_name     = $request->billing_name;
            $customer->billing_country  = $request->billing_country;
            $customer->billing_state    = $request->billing_state;

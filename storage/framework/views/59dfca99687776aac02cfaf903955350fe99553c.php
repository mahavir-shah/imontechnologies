                ...
                <div class="col-lg-5 ps-lg-0">
                    <div class="card m-0">
                        <div class="card-header p-2">
                            <div class="row">
                                <div class="col-md-6">
                                    <?php echo e(Form::text('customer_id', $customers->name,array('class' => 'form-control customer_select','id'=>'customer','required'=>'required', 'disabled'))); ?>
                                ...
                                <div class="col-md-6">
                                    <?php echo e(Form::select('customer_limit',$customer_limit,'', array('class' => 'form-control select customer_limit','required'=>'required'))); ?>

                                    <?php echo e(Form::hidden('customer_limit_hidden', '',['id' => 'customer_limit'])); ?>

                                    <input type="hidden" name="treadDiscount" id="treadDiscount" value="<?php echo e($treadDiscount); ?>"/>  
                                </div>
                            </div>
                        </div>
                        <div class="card-body carttable cart-product-list carttable-scroll"  id="carthtml">
                            <?php $total = 0; $treadDisApply = 0; ?>
                                <div class="table-responsive">
                                    <table class="table">
                                            ...
                                            <tr>
                                                ...
                                                <th class="text-center"><?php echo e(__('Price Total')); ?></th>
                                            ...
                                            <?php $__currentLoopData = session($lastsegment); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $id => $details): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                ...
                                                $image_url = (  !empty($product) && isset($product->pro_image) ) ? $product->pro_image : 'uploads/pro_image/';
                                                    ...
                                                    $treadDisApply += ((int)$details['price'] * (int)$details['quantity']) * $treadDiscount / 100;
                                                ?>
                                                    <tr data-product-id="<?php echo e($id); ?>" id="product-id-<?php echo e($id); ?>">
                                                        ...
                                                        <td class="priceTotal text-right"><?php echo e(Auth::user()->priceFormat(((int)$details['price'] * (int)$details['quantity']))); ?>

                                                            <input type="hidden" name="treadDisApply" value="<?php echo e(((int)$details['price'] * (int)$details['quantity']) * $treadDiscount / 100); ?>" class="treadDisApply"/>
                                                        </td>
                                    ...
                                    <div class="sub-total">
                                        <div class="d-flex justify-content-between">
                                            <div class="discountMain">
                                                <?php if($treadDisApply != 0): ?>
                                                    <h6 class="mb-0 text-dark totalDiscount"><?php echo e('Total Discount : '  . $treadDisApply); ?></h6>
                                                <?php endif; ?>
                                                <?php echo e(Form::hidden('discount', '',['id' => 'discountInput'])); ?>
...
<script>
    ...
    $( document ).ready(function() {
        ...
        $( "#customer_limit" ).val($('.customer_limit').val());
        var cust_limit = $('.customer_limit').val();
        if(cust_limit == 'credit_line'){
            $('.totalDiscount').text('Total Discount : 0.00');
        }
        $(function () {
        ...
        $(document).on('change', '.customer_limit', function (e) {
            var session_key =  $( "#empty_cart" ).val();
            
            Swal.fire({
                title: "This action can not be undone. Do you want to continue?",
                ...
                if (result.isConfirmed) {
                    ...
                    $( "#customer_limit" ).val(e.target.value);
                }else{
                    if(e.target.value == 'credit_line'){
                        $( ".customer_limit" ).val('advance_payment');
                        $( "#customer_limit" ).val('advance_payment');
                    }else{
                        $( ".customer_limit" ).val('credit_line');
                        $( "#customer_limit" ).val('credit_line');
                    }
                }
            })
        ...
        $(document).on('click', '.toacart', function () {
            ...
            var sum = 0;
            $( "#customer_limit" ).val($('.customer_limit').val());
            var customerLimit = $('#customer_limit').val();
            var treadDiscount = $('#treadDiscount').val();
            var totalDiscount = 0;
            $.ajax({
                url: $(this).data('url'),
                data:{
                ...
                success: function (data) {
                    ...
                    if (data.code == '200') {
                        ...
                        if ('carttotal' in data) {
                            $.each(data.carttotal, function (key, value) {
                                $('#product-id-' + value.id + ' .priceTotal').text(addCommas(parseInt(value.price) * parseInt(value.quantity)));
                                var priceTotal = parseInt(value.price) * parseInt(value.quantity);

                                if(customerLimit == 'advance_payment'){
                                    var treadAmt = priceTotal * parseInt(treadDiscount) / 100;
                                    $('#product-id-' + value.id + ' .treadDisApply').val(priceTotal * parseInt(treadDiscount) / 100);
                                    var tempPrice = priceTotal - treadAmt;
                                    var addtax = tempPrice * parseInt(value.tax) / 100;
                                    var finalsubtotal = tempPrice + addtax;
                                    totalDiscount += treadAmt;
                                    $('.totalDiscount').text('Total Discount : ' + totalDiscount.toFixed(2));
                                    $('#discountInput').val(totalDiscount.toFixed(2));
                                }else{
                                    var addtax = priceTotal * parseInt(value.tax) / 100;
                                    var finalsubtotal = priceTotal + addtax;
                                }
                                $('#product-id-' + value.id + ' .subtotal').text(addCommas(finalsubtotal));
                                sum += finalsubtotal;
                    ...
                    //    $('.discount').val('');
                        ...
                        $('.carttable #product-id-' + data.product.id + ' .priceTotal').text(addCommas(parseInt(data.product.price) * parseInt(data.product.quantity)));
                        // if(customerLimit == 'advance_payment'){
                            var priceTotal = parseInt(data.product.price) * parseInt(data.product.quantity);
                            $('.carttable #product-id-' + data.product.id + ' .treadDisApply').val(priceTotal * parseInt(treadDiscount) / 100);
                        // }
        ...
        $(document).on('click', '.btn-done-payment', function (e) {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    customer_limit : $('#customer_limit').val(),
                },
                beforeSend: function () {
                    ele.remove();
                },
...
<?php /**PATH /home/vsgpintl/admin.imontechnologies.in/resources/views/pos/index.blade.php ENDPATH**/ ?>
                        ...
                        <div data-v-f2a183a6="" class="d" id="boxes" style="margin-left: auto;margin-right: auto;">
                            <div data-v-f2a183a6="" class="d-body">
                                <div data-v-f2a183a6="" class="d-bill-to">
                                    <div class="row mb-10">
                                        <div class="bill_to">
                                            <strong data-v-f2a183a6="">{{__('Ship Detail')}}:</strong>
                                        ...
                                        <div class="bill_to cartonDetail">
                                            <strong data-v-f2a183a6="">{{__('Carton Detail')}}:</strong>
                                            <table data-v-f2a183a6="" class="summary-table">
                                                <tbody data-v-f2a183a6="">
                                                <tr>
                                                    <td>{{__('Delivery Type')}}:</td>
                                                    <td>{{!empty($pos->shipDetail->delivery)?$pos->shipDetail->delivery ? $pos->shipDetail->delivery : $pos->shipDetail->delivery:'-'}}</td>
                                                </tr>
                                                <tr>
                                                    <td>{{__('Weight')}}:</td>
                                                    <td>{{!empty($pos->shipDetail->weight)?$pos->shipDetail->weight:'-'}}</td>
                                                </tr>
                                                <tr>
                                                    <td>{{__('Height')}}:</td>
                                                    <td>{{!empty($pos->shipDetail->height)?$pos->shipDetail->height:'-'}}</td>
                                                </tr>
                                                </tbody>
                                            </table>
                                        </div>
                                    ...
                                    <hr/>
                                    <div class="row mt-10">
                                        @if(!empty($customers->billing_name))
                                            <div class="w-50">
                                                <strong>{{__('Billed To')}} :</strong><br>
                                                {{!empty($customers->billing_name)?$customers->billing_name:''}}<br>
                                                {{!empty($customers->billing_phone)?$customers->billing_phone:''}}<br>
                                                {{!empty($customers->billing_address)?$customers->billing_address:''}}<br>
                                                {{!empty($customers->billing_zip)?$customers->billing_zip:''}}<br>
                                                {{!empty($customers->billing_city)?$customers->billing_city:'' .', '}} {{!empty($customers->billing_state)?$customers->billing_state:'',', '}} {{!empty($customers->billing_country)?$customers->billing_country:''}}<br>
                                                <strong>{{__('Tax Number ')}} : </strong>{{!empty($customers->tax_number)?$customers->tax_number:''}}
                                            </div>
                                        @endif
                                    </div>
                                    <div class="row mt-10">
                                        @if(App\Models\Utility::getValByName('shipping_display')=='on')
                                            <div class="w-50 shipAddress pt-10">
                                                ...
                                                {{!empty($customers->shipping_name)?$customers->shipping_name:''}}<br>
                                                {{!empty($customers->shipping_phone)?$customers->shipping_phone:''}}<br>
                                                {{!empty($customers->shipping_address)?$customers->shipping_address:''}}<br>
                                                {{!empty($customers->shipping_zip)?$customers->shipping_zip:''}}<br>
                                                {{!empty($customers->shipping_city)?$customers->shipping_city:'' .', '}} {{!empty($customers->shipping_state)?$customers->shipping_state:'',', '}} {{!empty($customers->shipping_country)?$customers->shipping_country:''}}<br>
                                                <strong>{{__('Tax Number ')}} : </strong>{{!empty($customers->tax_number)?$customers->tax_number:''}}
                                            </div>
                                        @endif
                                    </div>

...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item">{{__('Goods Record')}}</li>
@endsection
@push('css-page')
    <style>
        .list-group-bg{
...
@push('script-page')
    <script>
        function goodRecordFilter(e, id, purchase_id) {
            ...
            $.ajax({
                url: '{{route('purchase.goodsFilter')}}',
                type: 'get',
                data: {
                    'good_id': goodId,
    ...
    {{-- <div class="row justify-content-between align-items-center mb-3">
        <div class="col-md-12 d-flex align-items-center justify-content-between justify-content-md-end">
            <div class="all-button-box">
                <a href="{{ route('purchase.goods_record_pdf', Crypt::encrypt($purchase->id))}}" target="_blank" class="btn btn-sm btn-primary">
                    {{__('Download')}}
                </a>
            </div>
        </div>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" value="{{ isset($getGoodsRecord[0]['received_date']) ? $getGoodsRecord[0]['received_date'] : ''}}" readonly>
                                    </div>
                                </div>
                                <div class="col-md-6">
        ...
        <div class="col-md-12">
            <div class="row">
                <div class="col-2 list-goods">
                    ...
                    @foreach ($getGoodId as $goods)
                        <div class="list-group list-group-flush my-2" id="useradd-sidenav">
                            <a href="#" onclick="goodRecordFilter(this,'{{$goods->goods_unique_id}}','{{$goods->purchase_id}}')" class="list-group-item list-group-item-action border-0 d-flex justifyContent list-group-bg"> 
                                <div class="left-menu">
                                    <span>{{$goods->goods_unique_id}}</span>
                                    <p>{{$goods->received_date}}</p>
                                </div>       
                            </a>
                        </div>
                    @endforeach
                ...
                <div class="col-1">
                    @foreach ($getGoodId as $goods)
                        <div class="pt-4 pb-3">
                            <a href="{{ route('purchase.goods_record_single_pdf',['unique_id' => $goods->goods_unique_id,'id' => $goods->purchase_id])}}" target="_blank" class="btn btn-primary">

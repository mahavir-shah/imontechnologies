                ...
                <div class="col-lg-5 ps-lg-0">
                    <div class="card m-0">
                        <div class="card-header p-2">
                            <div class="row">
                                <div class="col-md-6">
                                    {{ Form::text('customer_id', $customers->name,array('class' => 'form-control customer_select','id'=>'customer','required'=>'required', 'disabled')) }}
                                    {{ Form::hidden('vc_name_hidden', '',['id' => 'vc_name_hidden']) }}

                                </div>
                                <div class="col-md-6">
                                    {{ Form::select('customer_limit',$customer_limit,'', array('class' => 'form-control select customer_limit','required'=>'required')) }}
                                    {{ Form::hidden('customer_limit_hidden', '',['id' => 'customer_limit']) }}
                                    <input type="hidden" name="treadDiscount" id="treadDiscount" value="{{$treadDiscount}}"/>  
                                </div>
                            </div>
                        </div>
                        <div class="card-body carttable cart-product-list carttable-scroll"  id="carthtml">
                            @php $total = 0; $treadDisApply = 0; @endphp
                                <div class="table-responsive">
                                    <table class="table">
                                            ...
                                            <tr>
                                                ...
                                                <th class="text-center">{{__('Price Total')}}</th>
                                                <th>{{__('Tax')}}</th>
                                            ...
                                            @foreach(session($lastsegment) as $id => $details)
                                                ...
                                                @endphp
                                                    <tr data-product-id="{{$id}}" id="product-id-{{$id}}">
                                                        ...
                                                        <td class="priceTotal text-right">{{ Auth::user()->priceFormat(((int)$details['price'] * (int)$details['quantity'])) }}
                                                            <input type="hidden" name="treadDisApply" value="{{((int)$details['price'] * (int)$details['quantity']) * $treadDiscount / 100}}" class="treadDisApply"/>
                                                        </td>
                                    ...
                                    <div class="sub-total">
                                        <div class="d-flex justify-content-between">
                                            <div class="discountMain">
                                                @if ($treadDisApply != 0)
                                                    <h6 class="mb-0 text-dark totalDiscount">{{ 'Total Discount : '  . $treadDisApply }}</h6>
                                                @endif
                                                {{ Form::hidden('discount', '',['id' => 'discountInput']) }}
                                            </div>
